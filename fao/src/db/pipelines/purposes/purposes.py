...
CSV_PATH = get_csv_path_for("synthetic_lookups/purposes.csv")
...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.drop_duplicates(subset=['Purpose Code', 'source_dataset'], keep='first')
    
    # Drop any rows with null PKs
    df = df.dropna(subset=['Purpose Code'])
...
def insert(df: pd.DataFrame, session: Session):
    ...
    if records:
        try:
            stmt = pg_insert(Purposes).values(records)
