...
CSV_PATH = get_csv_path_for("synthetic_lookups/item_codes.csv")
...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.drop_duplicates(subset=['Item Code', 'source_dataset'], keep='first')
...
def insert(df: pd.DataFrame, session: Session):
    ...
    if records:
        try:
            stmt = pg_insert(ItemCodes).values(records)
