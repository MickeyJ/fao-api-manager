...
CSV_PATH = get_csv_path_for("synthetic_lookups/area_codes.csv")
...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.drop_duplicates(subset=['Area Code', 'source_dataset'], keep='first')
...
def insert(df: pd.DataFrame, session: Session):
    ...
    if records:
        try:
            stmt = pg_insert(AreaCodes).values(records)
