...
def get_fertilizers_detailed_trade_matrix(
    ...
    """
    Get fertilizers detailed trade matrix data with all related information from lookup tables.
    """
    
    # Build query - select all columns from main table
    query = select(FertilizersDetailedTradeMatrix)
        ...
        for table_data in row:
            if hasattr(table_data, '__table__'):
                ...
                for column in table_data.__table__.columns:
                    col_name = f"{table_name}_{column.name}" if table_name != "fertilizers_detailed_trade_matrix" else column.name
