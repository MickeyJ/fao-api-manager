...
def get_consumer_price_indices(
    ...
    """
    Get consumer price indices data with all related information from lookup tables.
    """
    
    # Build query - select all columns from main table
    query = select(ConsumerPriceIndices)
        ...
        for table_data in row:
            if hasattr(table_data, '__table__'):
                ...
                for column in table_data.__table__.columns:
                    col_name = f"{table_name}_{column.name}" if table_name != "consumer_price_indices" else column.name
