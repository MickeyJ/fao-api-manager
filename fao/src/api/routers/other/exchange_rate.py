...
def get_exchange_rate(
    ...
    """
    Get exchange rate data with all related information from lookup tables.
    """
    ...
    query = query.add_columns(Currencies)
        ...
        for table_data in row:
            if hasattr(table_data, '__table__'):
                ...
                for column in table_data.__table__.columns:
                    col_name = f"{table_name}_{column.name}" if table_name != "exchange_rate" else column.name
