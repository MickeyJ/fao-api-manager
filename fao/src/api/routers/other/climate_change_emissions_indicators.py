...
def get_climate_change_emissions_indicators(
    ...
    """
    Get climate change emissions indicators data with all related information from lookup tables.
    """
    
    # Build query - select all columns from main table
    query = select(ClimateChangeEmissionsIndicators)
        ...
        for table_data in row:
            if hasattr(table_data, '__table__'):
                ...
                for column in table_data.__table__.columns:
                    col_name = f"{table_name}_{column.name}" if table_name != "climate_change_emissions_indicators" else column.name
