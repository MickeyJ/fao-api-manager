...
def get_cost_affordability_healthy_diet_co_ahd(
    ...
    """
    Get cost affordability healthy diet co ahd data with all related information from lookup tables.
    """
    
    # Build query - select all columns from main table
    query = select(CostAffordabilityHealthyDietCoAhd)
    ...
    query = query.add_columns(Releases)
        ...
        for table_data in row:
            if hasattr(table_data, '__table__'):
                ...
                for column in table_data.__table__.columns:
                    col_name = f"{table_name}_{column.name}" if table_name != "cost_affordability_healthy_diet_co_ahd" else column.name
