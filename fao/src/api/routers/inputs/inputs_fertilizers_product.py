...
def get_inputs_fertilizers_product(
    ...
    """
    Get inputs fertilizers product data with all related information from lookup tables.
    """
    
    # Build query - select all columns from main table
    query = select(InputsFertilizersProduct)
        ...
        for table_data in row:
            if hasattr(table_data, '__table__'):
                ...
                for column in table_data.__table__.columns:
                    col_name = f"{table_name}_{column.name}" if table_name != "inputs_fertilizers_product" else column.name
