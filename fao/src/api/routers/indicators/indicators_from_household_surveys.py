...
def get_indicators_from_household_surveys(
    ...
    """
    Get indicators from household surveys data with all related information from lookup tables.
    """
    
    # Build query - select all columns from main table
    query = select(IndicatorsFromHouseholdSurveys)
        ...
        for table_data in row:
            if hasattr(table_data, '__table__'):
                ...
                for column in table_data.__table__.columns:
                    col_name = f"{table_name}_{column.name}" if table_name != "indicators_from_household_surveys" else column.name
