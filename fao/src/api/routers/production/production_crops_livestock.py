...
def get_production_crops_livestock(
    ...
    """
    Get production crops livestock data with all related information from lookup tables.
    """
    
    # Build query - select all columns from main table
    query = select(ProductionCropsLivestock)
        ...
        for table_data in row:
            if hasattr(table_data, '__table__'):
                ...
                for column in table_data.__table__.columns:
                    col_name = f"{table_name}_{column.name}" if table_name != "production_crops_livestock" else column.name
