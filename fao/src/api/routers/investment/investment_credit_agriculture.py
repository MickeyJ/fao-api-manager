...
def get_investment_credit_agriculture(
    ...
    """
    Get investment credit agriculture data with all related information from lookup tables.
    """
    
    # Build query - select all columns from main table
    query = select(InvestmentCreditAgriculture)
        ...
        for table_data in row:
            if hasattr(table_data, '__table__'):
                ...
                for column in table_data.__table__.columns:
                    col_name = f"{table_name}_{column.name}" if table_name != "investment_credit_agriculture" else column.name
