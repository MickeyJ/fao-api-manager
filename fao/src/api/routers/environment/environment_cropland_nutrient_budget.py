...
def get_environment_cropland_nutrient_budget(
    ...
    """
    Get environment cropland nutrient budget data with all related information from lookup tables.
    """
    
    # Build query - select all columns from main table
    query = select(EnvironmentCroplandNutrientBudget)
        ...
        for table_data in row:
            if hasattr(table_data, '__table__'):
                ...
                for column in table_data.__table__.columns:
                    col_name = f"{table_name}_{column.name}" if table_name != "environment_cropland_nutrient_budget" else column.name
